...
checks the live graph family's [GraphRules](graph-rules.md), adds admitted edges
...
See [networking configuration](networking.md#optional-chart-networking).
...
composition audit or [workload identity](workload-environment.md). Permission is
...
[connections and transport grants](networking.md#isolating-a-subgraph):
...
The [replication diagrams](replication.md#connections-between-copies) show how
...
[topology events](workload-events.md). Creating a Pending request alone does not
...
Composition and connection listeners share worker limits, the async bridge and
shutdown handling in `polyad.api.server`; their separate ports retain independent
enablement and authentication policies. `polyad.graph.temporary` defines the
