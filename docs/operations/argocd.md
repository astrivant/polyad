...
```sh
poetry run python scripts/gitops/argocd-health.py --format helm > polyad-argocd-values.yaml
```
...
```sh
poetry run python scripts/gitops/argocd-health.py > /tmp/polyad-argocd-patch.yaml
