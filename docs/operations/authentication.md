...
endpoint credentials, KEDA integration and External Secrets Operator.
...
See the [complete ReplicaGroup ScaledObject example](../graphs/replication.md#connect-keda).
...
replicas or clusters. The [graph rollout and rotation proposal](../proposals/rotations.md)
...
These single endpoint credentials are not refreshed in the running process.
Named API-key registries instead read projected token changes on the next request.
Inline Secret edits also
