...
Select `singular` or `ha` through the chart's top-level `tags`. These select the
operator deployment profile; `architecture.mode` and `rootControlPlane.enabled`
configure optional layouts within HA.
...
With no tag selected, the chart uses `ha`, preserving the default of two operator
replicas. Selecting both tags or an unknown tag is an error. Both default tag
values are false so `--set tags.singular=true` selects a profile without needing
to unset an implicit true flag.
...
tags. The HA reference enables cache replication; PostgreSQL is enabled only by
...
resource rendered by this release is for the management cluster; changing a tag
...
These are chart-defined profile tags. Helm's
[dependency tag mechanism](https://docs.helm.sh/docs/v3/chart_best_practices/dependencies/)
is separate: bundled dependencies remain controlled by their existing feature
flags. Tags never place resources in remote clusters by themselves.

When switching profiles, explicitly select one tag and clear the other. The
example values do this. A switch to singular also needs Dense mode, disabled
