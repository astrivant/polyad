...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("INFO", [], logging.INFO),
        ("debug", [], logging.DEBUG),
        ("DEBUG", ["--log-level", "WARNING"], logging.WARNING),
        ("WARNING", ["--debug"], logging.DEBUG),
        ("invalid", ["--debug"], logging.DEBUG),
    ],
)
@pytest.mark.parametrize("entrypoint", [runtime, observer])
def test_log_level_precedence_does_not_enable_transport_logging(monkeypatch, caplog, environment, arguments, level, entrypoint):
    """
    Scope verbose logging to Polyad and let explicit CLI settings override the environment.
    """
    if environment is None:
        monkeypatch.delenv("POLYAD_LOG_LEVEL", raising=False)
    else:
    ...
    observe = AsyncMock()
    monkeypatch.setattr(observer, "run", observe)
...
@pytest.mark.parametrize("entrypoint", [runtime, observer])
@pytest.mark.parametrize("arguments", [[], ["--debug", "--log-level", "INFO"]])
def test_invalid_environment_level_fails_before_starting_operator(monkeypatch, entrypoint, arguments):
    """
