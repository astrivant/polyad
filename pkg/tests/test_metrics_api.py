...
def test_graph_analysis_exports_certificates_spectra_and_fences_policy_changes(kind, monkeypatch):
    """
    ...
    for replica in ("replica-a", "replica-b"):
        ...
        store.publish(source, graph_labels=True)
        emitted = samples(store)
        assert {sample.labels["replica"] for sample in emitted} == {replica}
        eigenvalues = [s for s in emitted if s.name == "polyad_graph_eigenvalue"]
        assert len(eigenvalues) == 12
