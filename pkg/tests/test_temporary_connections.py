            ...
            return {
                "status": {
                    ...
                    "user": {
                        ...
                        "uid": self.caller.uid,
                        "groups": list(self.caller.groups),
                        "extra": self.caller.extra,
                    },
                }
            }
...
def participant(api, graph, node):
    """
    Create a UID-fenced service Pod for an existing logical endpoint.
    """
    name = f"participant-{graph['metadata']['name']}-{node}"
    account = resource("ServiceAccount", name)
    account["apiVersion"] = "v1"
    pod = resource("Pod", name, {"serviceAccountName": name})
    pod["apiVersion"] = "v1"
    for obj in (account, pod):
        obj["metadata"]["namespace"] = graph["metadata"]["namespace"]
        api.objects[obj["kind"], obj["metadata"]["namespace"], name] = obj
    pod["metadata"]["labels"] = {f"{asts.GROUP}/node": node}
    pod["metadata"]["ownerReferences"] = [
        {
            "apiVersion": graph["apiVersion"],
            "kind": graph["kind"],
            "name": graph["metadata"]["name"],
            "uid": graph["metadata"]["uid"],
            "controller": True,
        }
    ]
    namespace = graph["metadata"]["namespace"]
    return Caller(
        ...
        namespace,
        CALLER.groups,
        {"authentication.kubernetes.io/pod-name": [name], "authentication.kubernetes.io/pod-uid": [pod["metadata"]["uid"]]},
    )


async def approve_receipt(api, receipt):
    """
    Obtain real endpoint responses before testing admission and cleanup of an agreed connection.
    """
    stored = await api.get("TemporaryConnection", receipt["namespace"], receipt["name"])
    spec = stored["spec"]
    graph = await api.get(spec["kind"], spec["namespace"], spec["graph"])
    store = ConnectionStore(api, ConnectionSettings("test"))
    for node in (spec["source"], spec["target"]):
        caller = participant(api, graph, node)
        await store.respond(receipt["namespace"], receipt["name"], ConnectionResponse(receipt["uid"], "Approve"), caller)
...
def test_orphan_cleanup_preserves_other_receipts(monkeypatch):
    ...
    async def run():
        ...
        for agreed in receipts:
            await approve_receipt(api, agreed)
