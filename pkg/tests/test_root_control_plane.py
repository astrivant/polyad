...
def test_pool_install_upgrade_secret_rotation_and_scale_zero(monkeypatch, tmp_path, database):
    ...
    """
    from tests.test_work_graph_settings import CONFIGURATION, ENVIRONMENT
    ...
    async def scenario():
        ...
        source_env = root_api.objects[("Deployment", "test", "root")]["spec"]["template"]["spec"]["containers"][0]["env"]
        next(item for item in source_env if item["name"] == ENVIRONMENT["plannerParallelism"])["value"] = "3"
        pool = await root_api.get("OperatorPool", "test", "west")
        ...
        updated_env = {item["name"]: item.get("value") for item in updated["spec"]["template"]["spec"]["containers"][0]["env"]}
        assert updated_env[ENVIRONMENT["plannerParallelism"]] == "3"
        assert updated_env[ENVIRONMENT["reconciliationWorkers"]] == "4"
