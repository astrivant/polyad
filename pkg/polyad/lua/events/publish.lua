...
-- Bound deduplication memory independently of stream trimming; reset may allow harmless repeats.
if redis.call('HLEN', KEYS[2]) >= tonumber(ARGV[4]) and redis.call('HEXISTS', KEYS[2], ARGV[1]) == 0 then
    redis.call('DEL', KEYS[2])
end

-- Publish the event before recording its version, within the same atomic server operation.
