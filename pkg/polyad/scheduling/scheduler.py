"""
Schedule cooperative workloads while retaining resources until checkpoint completion.
"""
...
from polyad.scheduling import checkpoints
from polyad.scheduling.policy import ShortestRemaining
...
class Scheduler:
    ...
    def _emit(self, kind: str, **detail: object) -> None:
        ...
        if kind != "statistics":
            self.notify(f"[scheduling] {kind}: {json.dumps(detail)}")
        records = {
        ...
        if self.plots and kind in {"graph_created", "graph_changed"}:
            from polyad.scheduling.plotting import plot_graph
    ...
    def run(self, control: Control | None = None) -> dict[str, str]:
        ...
        try:
            ...
            pool = ThreadPoolExecutor(max_workers=self.slots, thread_name_prefix="scheduling")
