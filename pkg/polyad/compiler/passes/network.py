...
def _term(scope: NetworkScope, rule: TrafficRule) -> dict[str, Any]:
    ...
    return {
        "namespace": "" if peer.cluster else namespace,
        **({"cluster": peer.cluster} if peer.cluster else {}),
        "labels": labels,
...
def _intersect(left: dict[str, Any], right: dict[str, Any]) -> dict[str, Any] | None:
    if left["namespace"] != right["namespace"] or left.get("cluster") != right.get("cluster"):
        return None
...
def policy_specs(
    ...
    *,
    ...
    mesh_peers: dict[str, MeshPeer] | None = None,
    ...
    Args:
        ...
        mesh_peers (dict[str, MeshPeer] | None): Administrator-registered transport destinations.

    Returns:
        dict[str, dict[str, Any]]: Native policy specs keyed by resource kind.
    """
    result: dict[str, dict[str, Any]] = {}
    mesh_peers = mesh_peers or {}
    ...
    for direction, terms in (("ingress", ingress), ("egress", traffic(scopes, "egress"))):
        ...
        for term in terms:
            peer = None
            if term.get("cluster"):
                peer = mesh_peers.get(term["cluster"])
                if peer is None:
                    raise ValueError(f"remote mesh transport is not registered: {term['cluster']}")
                if direction == "egress" and peer.mode == "Gateway" and term["ports"] != [("TCP", peer.gatewayPort)]:
                    raise ValueError(
                        f"Gateway-mode egress grants the tunnel on TCP {peer.gatewayPort}; "
                        "service-port authorization belongs at the destination"
                    )
            rule: dict[str, Any] = {
            ...
            if peer is not None:
                if direction == "ingress" and peer.mode == "Gateway":
                    rule["from"] = [
                        {
                            "namespaceSelector": {"matchLabels": {"kubernetes.io/metadata.name": peer.gatewayNamespace}},
                            "podSelector": {"matchLabels": peer.gatewayLabels},
                        }
                    ]
                else:
                    rule["from" if direction == "ingress" else "to"] = [{"ipBlock": {"cidr": cidr}} for cidr in peer.cidrs]
            if term["ports"]:
    ...
    if any(scope.access.mesh for scope in scopes):
        ...
        for term in ingress or []:
            source = {} if term.get("cluster") else {"namespaces": [term["namespace"]]}
            if term["principals"]:
