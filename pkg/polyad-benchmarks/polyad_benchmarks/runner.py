...
def exercise(fixture_url: str, request_id: str, config: RunConfig, stop: threading.Event) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {"requestId": request_id, "phase": "Unknown"}
    try:
...
def run(
    ...
    return {
        ...
        "grafanaPath": f"/d/polyad-study?from={int(started_at.timestamp() * 1000)}&to={int(time.time() * 1000)}",
        "scheduled": scheduled,
...
def main() -> None:
    ...
    try:
        prefix = request_prefix(args.run_id or "")
        if args.command == "start":
            receipt = operator_client(10).activate(request_id=prefix, graph=args.graph, graph_uid=args.graph_uid, node="runner")
            print(json.dumps(receipt))
            return
        plan = (
            json.loads(Path(args.plan).read_text()) if args.plan else {"run": json.loads(os.environ.get("POLYAD_BENCHMARK_CONFIG", "{}"))}
        )
