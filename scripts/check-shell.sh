...
resolve_tool() {
    ...
    echo "$tool $expected required; no matching executable on PATH. Found: ${found:-none}. Run scripts/install-asdf-tools.sh $tool." >&2
    return 2
}
shellcheck_bin=$(resolve_tool shellcheck "$(bash scripts/tool-version.sh shellcheck)")
shfmt_bin=$(resolve_tool shfmt "$(bash scripts/tool-version.sh shfmt)")
files=("$@")
if [[ ${#files[@]} == 0 ]]; then
    manifest=$(mktemp)
