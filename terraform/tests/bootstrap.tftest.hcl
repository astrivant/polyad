...
run "gitops_bootstrap" {
  ...
  assert {
    condition = (
      ...
      yamldecode(helm_release.argocd.values[0]).global.nodeSelector["cloud.google.com/gke-nodepool"] == "polyad" &&
      yamldecode(helm_release.argocd.values[0]).global.tolerations[0].value == "polyad" &&
