...
| [`values-capabilities.reference.yaml`](references/values-capabilities.reference.yaml) | Opt-in TTL sharing contracts using existing publication and discovery listeners | [Capability contracts](../../docs/workloads/capability-contracts.md) |
| [`values-cheeger.reference.yaml`](references/values-cheeger.reference.yaml) | Opt-in Cheeger reduction with certificate-gap or computation-time PID feedback and soft accuracy/time targets | [Cheeger tuning](../../docs/graphs/cheeger-tuning.md) |
...
| `operator.cheeger.reduction.enabled`                                 | **Type: boolean.** Allow graph policies to request reduced Cheeger search before exact enumeration                                                                                                                                       | `false` |
| `operator.cheeger.reduction.maxVertices`                             | **Type: integer.** Largest boundary allowed through the dense spectral reducer (2-4096)                                                                                                                                                  | `256` |
| `operator.cheeger.reduction.components`                              | **Type: integer.** Maximum nontrivial Laplacian eigenvectors retained by a graph policy (1-64)                                                                                                                                           | `4` |
| `operator.cheeger.reduction.supernodes`                              | **Type: integer.** Maximum quotient vertices searched by a graph policy (2-64)                                                                                                                                                           | `8` |
| `operator.cheeger.reduction.cache`                                   | **Type: boolean.** Permit process-local quotient partition reuse; every cached partition is rescored on current edges                                                                                                                    | `true` |
| `operator.cheeger.reduction.cacheEntries`                            | **Type: integer.** Maximum cached quotient partitions per operator process (1-4096)                                                                                                                                                      | `128` |
| `operator.cheeger.reduction.maxEdgeChurn`                            | **Type: number.** Maximum symmetric changed-edge fraction eligible for cached partition reuse (0-1)                                                                                                                                      | `0.1` |
| `operator.cheeger.reduction.strategy`                                | **Type: string.** Preferred scheduler after reduction opt-in: AdaptivePID adjusts refresh and cache-use target; CacheFirst restores legacy ordering                                                                                      | `AdaptivePID` |
| `operator.cheeger.reduction.targetSeconds`                           | **Type: number.** Soft mean computation-time goal for adaptive cache targeting (0.000001-300 seconds); not a timeout or CPU reservation                                                                                                  | `0.0015` |
| `operator.cheeger.reduction.feedback`                                | **Type: string.** Adaptive PID feedback: CertificateGap targets certified uncertainty; ComputationTime retains latency feedback                                                                                                          | `CertificateGap` |
| `operator.cheeger.reduction.targetRelativeError`                     | **Type: number.** Soft certified relative-error goal (0.000001-100); 0.25 means 25 percent, not a hard accuracy guarantee                                                                                                                | `0.25` |
